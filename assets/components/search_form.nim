...
proc SearchHolder*(placeholder: string = "Найти преподавателя"): TagRef =
  buildHtml:
    tDiv(
      ...
      tDiv(
        class = fmt"flex flex-col gap-6 items-center pt-4 rounded-md text-[{Fg}] duration-300 transition-all " & (
        ...
        tSpan(class = "text-xl font-semibold"):
          "Поиск"
        tInput(
          class = fmt"rounded-md bg-[{BgMain}] mx-2 px-4 py-1 placeholder:text-[{Fg}] text-[{Fg}]",
        ...
        tDiv(class = "flex w-full"):
          tDiv(class = fmt"w-full font-bold px-4 py-2 rounded-bl-md bg-[{BgMain}] text-[{Primary}] cursor-pointer select-none duration-300 transition-all"):
            "отмена"
            @click:
              search.val = ""
              showSearchForm.set(false)
          tDiv(class = fmt"w-full font-bold text-right px-4 py-2 rounded-br-md bg-[{Primary}] text-[{Bg}] cursor-pointer select-none duration-300 transition-all"):
            "поиск"
            @click:
              showSearchForm.set(false)
